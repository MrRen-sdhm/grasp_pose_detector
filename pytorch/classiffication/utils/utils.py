...
def save_checkpoint(state, is_best, fold):
    filename = config.weights + config.model_name + os.sep + fold + os.sep + "_checkpoint.pt"
    torch.save(state, filename)
    if is_best:
        best_precision1 = state["best_precision1"].cpu().data.numpy()
        message = config.best_models + config.model_name + os.sep + fold + os.sep + 'model_best_%.2f.pt' % best_precision1
        print("Top1[%.2f] better model:%s" % (best_precision1, message))
        with open("./logs/%s_%s.txt" % (config.model_name, config.fold),"a") as f:
