...
net.load_state_dict(torch.load('model_mobilenet_85.1.pwf'))
print(net)
 ...
 #   scheduler.step()
    ...
    for j in range(iter_per_epoch):
        ...
        for r in range(repeats):
            ...
            # accuracy = eval(net, test_loader, device)
            # accuracies.append(accuracy)
            # model_path = 'model_mobilenet' + str(accuracy) + '.pwf'
    ...
    if epoch % 1 == 0:
        accuracy = eval(net, test_loader, device)
        accuracies.append(accuracy)
        model_path = 'model_mobilenet_' + sys.argv[3] + 'ch_' + '%.2f' % accuracy + '.pwf'
        torch.save(net.state_dict(), model_path)
