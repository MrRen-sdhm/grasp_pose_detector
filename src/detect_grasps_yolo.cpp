    ...
    namespace apps {
        namespace detect_grasps {
            ...
            int DoMain(int argc, char *argv[]) {
                ...
                int inpWidth = 416;  // Width of network's input image 416
                int inpHeight = 416; // Height of network's input image 416
                std::string classes[80]={"person","bicycle","car","motorbike","aeroplane","bus","train","truck","boat","traffic light","fire hydrant","stop sign",
                ...
                std::vector<std::string> classes_vec(classes, classes+80);

                // net init
                cv::dnn::Net net;
                net = cv::dnn::readNetFromDarknet(yolo_config_filename, yolo_weights_filename);
                ...
                // 调用yolo
                std::vector<cv::Rect> obj_boxes;
                cv::dnn::blobFromImage(image, blob, 1/255.0, cvSize(inpWidth, inpHeight), cv::Scalar(0,0,0), true, false); // Create a 4D blob from a frame.
                ...
                obj_boxes = postprocess(image, outs, confThreshold, nmsThreshold, classes_vec); // Remove the bounding boxes with low confidence
