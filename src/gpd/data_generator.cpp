...
namespace gpd {
    ...
    DataGenerator::DataGenerator(const std::string &config_filename)
        ...
        raw_data_root_ = config_file.getValueOfKeyAsString("raw_data_root", "");
        ...
        std::cout << "raw_data_root: " << raw_data_root_ << "\n";
    ...
    void DataGenerator::generateDataBigbird() {
        ...
        int store_step = 1; // 每store_step个物体保存一次
...
//        printf("train_data size:%zu test_data size:%zu\n", train_data.size(), test_data.size()); // 实际大小
        ...
        for (int i = 0; i < num_objects; i++) {
            ...
            util::Cloud mesh = loadMesh(prefix + "_gt.pcd", prefix + "_gt_normals.csv"); // name: /home/.../datasets/object/object_gt.pcd /home/.../datasets/object/object_gt_normals.csv
            ...
            for (int j = 0; j < num_views_per_object_; j++) {
                ...
                view_points << 0.0, 0.0, 0.0;  // NOTE: Load camera position.
                util::Cloud cloud(
                        prefix + "_" + boost::lexical_cast<std::string>(j + 1) + ".pcd", // name: /home/.../datasets/object/object_1.pcd
                        view_points);
                cloud.voxelizeCloud(VOXEL_SIZE);
                cloud.calculateNormalsOMP(num_threads_);
                cloud.setNormals(cloud.getNormals() * (-1.0)); // NOTE: 翻转单视角点云表面法线（坐标系在物体内部时使用）
...
//        num_views_per_object_ = 4;
        ...
        for (int i = 0; i < num_objects; i++) {
            ...
            for (int j = 0; j < num_views_per_object_; j++) {
                ...
                cloud.voxelizeCloud(VOXEL_SIZE);
                cloud.calculateNormalsOMP(num_threads_);
                cloud.setNormals(cloud.getNormals() * (-1.0)); // NOTE: 翻转单视角点云表面法线
...
// src: multi-channels image; dst: multi-dimensional matrix
    ...
    util::Cloud DataGenerator::createMultiViewCloud(const std::string &object,
        ...
        const std::string prefix = raw_data_root_ + object;
        ...
        for (int i = 0; i < angles.size(); i++) {
            ...
            if (access(cloud_filename.c_str(), R_OK) != 0) { // 文件不存在
                printf("\033[0;36m%s\033[0m\033[0;36m%s\033[0m\n", "[INFO] Can't read pcd file: ", cloud_filename.c_str());
                continue;
            }
            ...
            printf("proc_cloud_filename: %s\n", (prefix + "/multi_view_clouds/" + object + np_str + std::to_string(angles[i]) + ".pcd").c_str());
            pcl::io::savePCDFileASCII(
                    prefix + "/multi_view_clouds/" + object + np_str + std::to_string(angles[i]) + ".pcd", *cloud);
        ...
        printf("merged_cloud_filename: %s\n", (prefix + "/multi_view_clouds/" + object + np_str + "_merged.pcd").c_str());
        pcl::io::savePCDFileASCII(prefix + "/multi_view_clouds/" + object + np_str + "_merged.pcd",
    ...
    Eigen::Matrix4f DataGenerator::calculateTransform(const std::string &object,
                                                      ...
                                                      int reference_camera) const {
        std::string pose_filename = raw_data_root_ + object + "/poses/NP" +
        ...
        std::string calibration_filename = raw_data_root_ + object + "/calibration.h5";
