...
void Image15ChannelsStrategy::saveImage(const cv::Mat &image) const  {
  int border = 5;
  int n = 3;
  int image_size = image_params_.size_;
  int total_size = n * (image_size + border) + border;
  ...
  std::string image_save_name = "/home/sdhm/图片/images/GraspImage";
  image_save_name = image_save_name + std::to_string(image_cnt) + ".jpg";
  printf("save image:%s\n", image_save_name.c_str());
  cv::imwrite(image_save_name, image_out);
